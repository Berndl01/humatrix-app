...
create type team_role as enum ('player', 'coach');
create type questionnaire_type as enum ('player_selftest', 'coach_player_rating', 'coach_selftest');
create type question_type as enum ('scale', 'single_choice', 'multiple_choice', 'text');
create type submission_status as enum ('draft', 'submitted');
create type player_type_code as enum (
  'ESAF', 'ESAD', 'ESIF', 'ESID',
  'ETAF', 'ETAD', 'ETIF', 'ETID',
  'WSAF', 'WSAD', 'WSIF', 'WSID',
  'WTAF', 'WTAD', 'WTIF', 'WTID'
...
-- 13. TYPE RESULTS
-- ─────────────────────────────────────────────────────────────────────
create table public.type_results (
  id uuid primary key default uuid_generate_v4(),
  submission_id uuid not null references public.survey_submissions(id) on delete cascade,
...
create index idx_player_profiles_team on public.player_profiles(team_id);
create index idx_survey_submissions_subject on public.survey_submissions(subject_user_id);
create index idx_survey_submissions_by on public.survey_submissions(submitted_by);
create index idx_survey_answers_submission on public.survey_answers(submission_id);
create index idx_type_results_user on public.type_results(user_id);
create index idx_coach_feedback_player on public.coach_feedback(player_user_id);
...
-- ─────────────────────────────────────────────────────────────────────
create table public.player_type_definitions (
...
create table public.battery_responses (
  id uuid primary key default uuid_generate_v4(),
  user_id uuid not null references public.profiles(id) on delete cascade,
  team_id uuid references public.teams(id),
...
create index idx_battery_user on public.battery_responses(user_id);
create index idx_battery_team on public.battery_responses(team_id);
